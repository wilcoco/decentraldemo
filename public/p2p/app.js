...
let ledgerSeen = null; // 장부에 이미 보인 항목 — 새 항목만 흘러 들어오는 애니메이션
let ledgerVerifyMsg = ''; // 재검증 결과 (재렌더에도 남는다)
let pendingDelegate = null; // 위임 확인 단계 (맡기기 전에 수임자의 행보를 본다)
...
function renderOpinions() {
  ...
  const op = $('#opinions');
  const keepScroll = op.scrollLeft; // 재렌더가 읽던 위치를 뺏지 않도록
  op.innerHTML = opinions.length
    ? cols.join('')
    : `<div class="card empty-guide" style="flex:1"><h3>아직 의견이 없습니다</h3><p class="hint">이 이슈에 대한 <b>첫 의견(해결책·주장)</b>을 아래에서 제안해 보세요.<br/>제안한 의견에는 다른 시민들이 논거를 붙여 지지하거나 반대하게 됩니다.</p></div>` +
      cols[cols.length - 1];
  op.scrollLeft = keepScroll;
  renderColDots();
}

// 모바일 스와이프 인디케이터: 열 하나가 화면 하나 — 점이 현재 위치를 알려준다
function renderColDots() {
  const n = $('#opinions').querySelectorAll('.kanban-col').length;
  $('#col-dots').innerHTML =
    n > 1 ? Array.from({ length: n }, (_, i) => `<button class="col-dot" data-col-dot="${i}" aria-label="${i + 1}번째 의견"></button>`).join('') : '';
  updateColDots();
}
function updateColDots() {
  const op = $('#opinions');
  const dots = document.querySelectorAll('.col-dot');
  if (!dots.length) return;
  const cols = op.querySelectorAll('.kanban-col');
  // 현재 열 = 시작점이 스크롤 위치에 가장 가까운 열
  let idx = 0;
  let best = Infinity;
  for (let i = 0; i < cols.length; i++) {
    const d = Math.abs(cols[i].offsetLeft - op.scrollLeft);
    if (d < best) {
      best = d;
      idx = i;
    }
...
function renderMyBar() {
  ...
  // 위임 선택 (다운스) — 맡기기에는 확인 단계(수임자의 행보 미리보기),
  // 회수(직접 참여)는 마찰 없이 즉시. 쉬운 것은 되찾는 쪽이어야 한다.
  const sel = $('#delegate-select');
  const others = [...node.registry.keys()].filter((c) => c !== wallet.citizenId);
  const shown = pendingDelegate ?? currentDelegation;
  sel.innerHTML =
    '<option value="">직접 참여 (기본)</option>' +
    others.map((c) => `<option value="${c}" ${c === shown ? 'selected' : ''}>${esc(nameOf(c))}에게 맡기기</option>`).join('');
  sel.onchange = () => {
    if (!sel.value) {
      pendingDelegate = null;
      mesh.act(currentTopic, 'DELEGATE', { to: null }); // 회수는 즉시
    } else if (sel.value !== currentDelegation) {
      pendingDelegate = sel.value; // 맡기기는 행보를 보고 나서
      render();
    }
  };
  $('#delegation-help').innerHTML = info('위임') + helpBox(['위임']);

  const confirmEl = $('#delegate-confirm');
  if (pendingDelegate) {
    const hub = computeInsight(node).citizenHub.get(pendingDelegate);
    const theirOp = opinions.find(
      (o) => o.standers.includes(pendingDelegate) || o.opposers.includes(pendingDelegate)
    );
    const theirSide = theirOp ? (theirOp.standers.includes(pendingDelegate) ? '지지' : '반대') : null;
    const acts = node.entriesForTopic(currentTopic).filter((e) => e.author === pendingDelegate).length;
    confirmEl.innerHTML = `<div class="delegate-confirm">
      🗳 <b>${esc(nameOf(pendingDelegate))}</b>에게 맡기기 전에, 이 사람의 행보를 확인하세요:
      <div class="hint">• 이 이슈에서: ${
        theirOp ? `“${esc(theirOp.title)}” <b>${theirSide}</b> 줄에 서 있음` : '아직 어느 줄에도 서지 않음'
      } · 행위 ${acts}건</div>
      <div class="hint">• 안목 지수 ${info('안목')}: ${hub != null ? hub.toFixed(2) : '기록 없음'}</div>
      <div class="hint">내 표는 이 사람이 서는 곳에 함께 실립니다. <b>내가 직접 서는 순간 위임은 물러나고</b>, 언제든 회수할 수 있습니다.</div>
      <button class="btn primary small" data-delegate-confirm>이 사람에게 맡기기</button>
      <button class="btn ghost small" data-delegate-cancel>취소</button>
    </div>`;
  } else confirmEl.innerHTML = '';
}
...
function ledgerLine(e, isNew = false) {
  ...
  const fork = node.forkProofs.has(e.author)
    ...
    : '';
  return `<div class="ledger-ev${node.forkProofs.has(e.author) ? ' fork' : cls}${isNew ? ' new' : ''}">${base}${witness}${fork}
...
function renderLedger() {
  ...
  const entries = node
    ...
    .sort((a, b) => b.ts - a.ts)
    .slice(0, 60);
  // 실시간 흐름: 연 뒤에 도착한 항목만 미끄러져 들어온다 (열 때 전체가 요동치지 않게)
  const firstOpen = ledgerSeen === null;
  if (firstOpen) ledgerSeen = new Set();
  const rows = entries.map((e) => ledgerLine(e, !firstOpen && !ledgerSeen.has(e.hash)));
  for (const e of entries) ledgerSeen.add(e.hash);
  el.innerHTML = `<h3>📜 위브 장부 ${info('장부')} <span class="ledger-live" title="실시간 — 새 행위가 도착하는 즉시 나타납니다"></span></h3>
    <div class="ledger-head">참여 내역은 공개 장부입니다 — 누구나 같은 기록을 갖고 서로 검증합니다.<br/>
    이 장부 <b>${entries.length}건</b>은 저장 시점에 전부 서명 검증되었습니다.
    <button class="btn small" data-ledger-verify>지금 다시 검증</button> <span id="ledger-verify-msg">${esc(ledgerVerifyMsg)}</span></div>
    ${helpBox(['장부'])}
    ${rows.join('') || '<p class="hint">아직 기록이 없습니다.</p>'}`;
}

async function runLedgerVerify() {
  ledgerVerifyMsg = '검증 중…';
  const msgEl = $('#ledger-verify-msg');
  if (msgEl) msgEl.textContent = ledgerVerifyMsg;
  ...
  ledgerVerifyMsg = bad === 0 ? `✓ 방금 이 기기에서 ${ok}건 재검증 완료 (${ms}ms)` : `⚠ ${bad}건 검증 실패 — 네트워크에 알려집니다`;
  const el = $('#ledger-verify-msg');
  if (el) el.textContent = ledgerVerifyMsg;
}
...
function render0penForm() {
  ...
  const op = $('#opinions');
  const keepScroll = op.scrollLeft;
  op.innerHTML = cols.join('');
  op.scrollLeft = keepScroll;
  renderColDots();
}
...
document.addEventListener('click', (e) => {
  ...
  const dot = hit('[data-col-dot]');
  if (dot) {
    const op = $('#opinions');
    const col = op.querySelectorAll('.kanban-col')[Number(dot.dataset.colDot)];
    if (col) {
      op.scrollLeft = col.offsetLeft; // 즉시 이동 — 1초 재렌더와 경합하지 않게
      updateColDots();
    }
    return;
  }
  if (hit('[data-delegate-confirm]')) {
    const to = pendingDelegate;
    pendingDelegate = null;
    if (to) mesh.act(currentTopic, 'DELEGATE', { to }).then(render);
    return;
  }
  if (hit('[data-delegate-cancel]')) {
    pendingDelegate = null;
    render();
    return;
  }
...
$('#opinions').addEventListener('scroll', updateColDots, { passive: true });
